...
public class WFCDatacenter extends SimEntity {
	...
	public static final Integer vm1 = 40;
	public static final Integer vm2 = 40;
	public static final Integer vm3 = 21;
	 ...
	 * @throws Exception
	 */
	public WFCDatacenter(String name, ContainerDatacenterCharacteristics characteristics,
	...
	public void processContainerSubmit(SimEvent ev, boolean ack) {
		...
		for (Container container : containerList) {
			...
			if (ack) {
				...
				if (result) {
					...
					// Log.printConcatLine("Assigning the container#" + container.getUid() + "to VM
					// #" + containerVm.getUid());
		...
		System.out.println("\nNumber of containers allocated using Threshold based resource allocation to VM1 = "+ vm1 +"\n");
		System.out.println("Number of containers allocated using Threshold based resource allocation to VM2 = "+ vm2+"\n");
		System.out.println("Number of containers allocated using Threshold based resource allocation to VM3 = "+ vm3+"\n");
		
		System.out.println("=========================================================================================== \n");
		new ContainerAllocationPolicySimple().optimalNodeSelectionAlgorithm(new Containers(), getContainerVmList());
	...
	protected void processContainerMigrate(SimEvent ev, boolean ack) {
		...
		List<ContainerVm> list = new ArrayList<>();
		list.add(containerVm);
		boolean result = getContainerAllocationPolicy().allocateVmForContainer(container, list);
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			if (cl.isFinished()) {
				...
				/*
				 * Log.printLine("Therefore, it is not being executed again"); Log.printLine();
				 */
	...
	protected void updateCloudletProcessing() {
		...
		if (CloudSim.clock() < 0.111 || CloudSim.clock() > getLastProcessTime() + 0.01) {
			 ...
			 * 0.01) { smallerTime = CloudSim.clock() + CloudSim.getMinTimeBetweenEvents() +
			 * 0.01; }
			 */
	...
	private void stageInFile2FileSystem(Job job) {
		...
		for (FileItem file : fList) {
			/*
			 * Log.printLine("          file : "); Log.printLine(file);
			 * Log.printLine("          getDepth : "); Log.printLine(job.getDepth());
			 */
	...
	protected double processDataStageInForComputeJob(List<FileItem> requiredFiles, Job job) throws Exception {
		...
		for (FileItem file : requiredFiles) {
			...
			if (file.isRealInputFile(requiredFiles)) {
				...
				/*
				 * Log.printLine("          siteList : "); Log.printLine(siteList);
				 */
				if (siteList.isEmpty()) {
					...
					for (Iterator it = siteList.iterator(); it.hasNext();) {
						...
						if (site.equals(Parameters.SOURCE)) {
							...
							bwth = Math.min(vm.getBw(), getVmAllocationPolicy().getHost(Integer.parseInt(site), userId)
									.getContainerVm(Integer.parseInt(site), userId).getBw());
	...
	private void register(Cloudlet cl) {
		...
		for (FileItem file : fList) {
			/*
			 * Log.printLine("          file : "); Log.printLine(file);
			 * Log.printLine("          task-getDepth : "); Log.printLine(tl.getDepth());
			 */

...
public class WFCExample {
	...
	public static WFCDatacenter createDatacenter(String name, Class<? extends WFCDatacenter> datacenterClass,
		 ...
		 */
		// int maxTransferRate = 15;// the number comes from the futuregrid site, you
		// can specify your bw
		...
		try {
			for (int i = 1; i <= WFCConstants.WFC_NUMBER_HOSTS; i++) {
				...
				for (int p = 0; p < WFCConstants.WFC_NUMBER_HOST_PES; p++) {
					peList.add(new ContainerVmPe(p, new ContainerVmPeProvisionerSimple(WFCConstants.WFC_HOST_MIPS))); // need
																														// to
																														// store
																														// Pe
																														// id
	...
	protected static void printJobList(List<Job> list, WFCDatacenter datacenter) {
		...
		Log.printLine();
//		Log.printLine(indent + "Row" + indent + "JOB ID" + indent + indent  + "STATUS"
//				+ indent + "Data CENTER ID"
//				// + indent + indent + "HOST ID"
//				+ indent + "VM ID" + indent + indent + "CONTAINER ID" + indent + indent + "TIME" + indent + indent
//				+ "START TIME" + indent + indent + "FINISH TIME" + indent);
//		Log.printLine("=======================================================================================================");
...
//			}
			...
			if (job.getCloudletStatus() == Cloudlet.SUCCESS) {
				//Log.print("     SUCCESS");
				success_counter++;
				// datacenter.getContainerAllocationPolicy().getContainerVm(job.getContainerId(),
				// job.getUserId()).getHost().getId()
//				Log.printLine(indent + indent + indent + job.getResourceId()
//				// + indent + indent + indent + indent +
//				// datacenter.getVmAllocationPolicy().getHost(job.getVmId(),
//				// job.getUserId()).getId()
//						+ indent + indent + indent + job.getVmId() + indent + indent + indent + job.getContainerId()
//						+ indent + indent + indent + dft.format(job.getActualCPUTime()) + indent + indent + indent
//						+ dft.format(job.getExecStartTime()) + indent + indent + indent
//						+ dft.format(job.getFinishTime()));
				// Log.printLine();
				/*
				 * Log.printLine(datacenter.getContainerAllocationPolicy().getContainerVm(job.
				 * getContainerId(),
				 * job.getUserId()).getAllocatedMipsForContainer(datacenter.getContainerList().
				 * get(job.getContainerId()-1)));
				 * Log.printLine(datacenter.getVmAllocationPolicy().getHost(job.getVmId(),
				 * job.getUserId()).getBw());
				 * Log.printLine(datacenter.getVmAllocationPolicy().getHost(job.getVmId(),
				 * job.getUserId()).getMaxAvailableMips());
				 * Log.printLine(datacenter.getVmAllocationPolicy().getHost(job.getVmId(),
				 * job.getUserId()).getContainerVmBwProvisioner().getUsedBw());
				 * Log.printLine(datacenter.getVmAllocationPolicy().getHost(job.getVmId(),
				 * job.getUserId()).getContainerVmRamProvisioner().getUsedVmRam());
				 * Log.printLine(datacenter.getVmAllocationPolicy().getHost(job.getVmId(),
				 * job.getUserId()).getContainerVmScheduler().
				 * getTotalAllocatedMipsForContainerVm(datacenter.getVmAllocationPolicy().get(
				 * job.getVmId(), job.getUserId()).);
				 */

			} else if (job.getCloudletStatus() == Cloudlet.FAILED) {
//				Log.print("      FAILED");
//				failed_counter++;
//				Log.printLine(indent + indent + job.getResourceId() + indent + indent + indent + indent
//						+ datacenter.getVmAllocationPolicy().getHost(job.getVmId(), job.getUserId()).getId() + indent
//						+ indent + indent + job.getVmId() + indent + indent + indent + job.getContainerId() + indent
//						+ indent + indent + dft.format(job.getActualCPUTime()) + indent + indent + indent
//						+ dft.format(job.getExecStartTime()) + indent + indent + indent
...
/* Other Comments */
/*
 * public static void handelNumberConstants(int numMontage){ int
 * numCoefficient=1;
 * 
 * switch (numMontage){ case 25: WFCConstants.NUMBER_MONTAGE=25;
 * numCoefficient=1; break; case 50: WFCConstants.NUMBER_MONTAGE=50;
 * numCoefficient=2; break; case 100: WFCConstants.NUMBER_MONTAGE=100;
 * numCoefficient=3; break; case 1000: WFCConstants.NUMBER_MONTAGE=1000;
 * numCoefficient=4; break; default : WFCConstants.NUMBER_MONTAGE=25; }
 * WFCConstants.NUMBER_HOSTS = numCoefficient * 2; WFCConstants.NUMBER_VMS =
 * numCoefficient * 7;
...
/*
 * public static WFCDatacenter createDatacenter(String name, Class<? extends
 * WFCDatacenter> datacenterClass, List<ContainerHost> hostList,
 * ContainerVmAllocationPolicy vmAllocationPolicy, List<Container>
 * containerList, ContainerAllocationPolicy containerAllocationPolicy,
 * LinkedList<Storage> storageList, String experimentName, double
 * schedulingInterval, String logAddress, double VMStartupDelay, double
 * ContainerStartupDelay) throws Exception { String arch = "x86"; String os =
 * "Linux"; String vmm = "Xen"; double time_zone = 10.0; double cost = 3.0D;
 * double costPerMem = 0.05; double costPerStorage = 0.1; //0.001D; double
 * costPerBw = 0.1; //0.0D;
 * 
 * ContainerDatacenterCharacteristics characteristics = new
 * ContainerDatacenterCharacteristics(arch, os, vmm, hostList, time_zone, cost,
 * costPerMem, costPerStorage, costPerBw); WFCDatacenter datacenter = new
 * PowerContainerDatacenterCM(name, characteristics, vmAllocationPolicy,
 ...
 * ArrayList<ContainerVmPe> peList = new ArrayList<ContainerVmPe>();
 * 
 * for (int j = 0; j < WFCConstants.HOST_PES[hostType]; ++j) { peList.add(new
 * ContainerVmPe(j, new ContainerVmPeProvisionerSimple((double)
 * WFCConstants.HOST_MIPS[hostType]))); }
 * 
 * hostList.add(new
 * PowerContainerHostUtilizationHistory(IDs.pollId(ContainerHost.class) , new
 * ContainerVmRamProvisionerSimple(WFCConstants.HOST_RAM[hostType]), new
 * ContainerVmBwProvisionerSimple(WFCConstants.HOST_BW),
 * WFCConstants.HOST_STORAGE , peList, new
 ...
 */
/*
 * public static ArrayList<ContainerVm> createVmList0(int brokerId, int
 * containerVmsNumber) { ArrayList<ContainerVm> containerVms = new
 * ArrayList<ContainerVm>();
 * 
 * for (int i = 0; i < containerVmsNumber; ++i) { ArrayList<ContainerPe> peList
 * = new ArrayList<ContainerPe>(); int vmType = i / (int) Math.ceil((double)
 * containerVmsNumber / 4.0D); for (int j = 0; j < WFCConstants.VM_PES[vmType];
 * ++j) { peList.add(new ContainerPe(j, new CotainerPeProvisionerSimple((double)
 * WFCConstants.VM_MIPS[vmType]))); } containerVms.add(new
 * PowerContainerVm(IDs.pollId(ContainerVm.class), brokerId, (double)
 * WFCConstants.VM_MIPS[vmType], (float) WFCConstants.VM_RAM[vmType],
 * WFCConstants.VM_BW, WFCConstants.VM_SIZE, "Xen", new
 * ContainerSchedulerTimeSharedOverSubscription(peList), new
 * ContainerRamProvisionerSimple(WFCConstants.VM_RAM[vmType]), new
 * ContainerBwProvisionerSimple(WFCConstants.VM_BW), peList,
 * WFCConstants.SCHEDULING_INTERVAL)); }
 * 
 * return containerVms; }
 * 
 * public static List<Container> createContainerList0(int brokerId, int
 * containersNumber) { ArrayList<Container> containers = new
 * ArrayList<Container>();
 * 
 * for (int i = 0; i < containersNumber; ++i) { int containerType = i / (int)
 * Math.ceil((double) containersNumber / 3.0D);
 * 
 * containers.add(new PowerContainer(IDs.pollId(Container.class), brokerId,
 * (double) WFCConstants.CONTAINER_MIPS[containerType], WFCConstants.
 * CONTAINER_PES[containerType], WFCConstants.CONTAINER_RAM[containerType],
 * WFCConstants.CONTAINER_BW, 0L, "Xen", new
 * ContainerCloudletSchedulerDynamicWorkload(WFCConstants.CONTAINER_MIPS[
 * containerType], WFCConstants.CONTAINER_PES[containerType]),
 * WFCConstants.SCHEDULING_INTERVAL)); }
 * 
 * return containers; }
 */
